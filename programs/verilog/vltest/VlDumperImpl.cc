...
VlDumperImpl::put(const VlMgr& mgr)
{
  ...
  while ( !tmp_queue.empty() ) {
    auto module = tmp_queue.front();
    tmp_queue.pop();
    put_module("MODULE", mgr, module);

    auto module_list = mgr.find_module_list(module);
    for ( auto module1: module_list ) {
      tmp_queue.push(module1);
    }
    auto modulearray_list = mgr.find_modulearray_list(module);
    for ( auto module_array: modulearray_list ) {
      auto n = module_array->elem_num();
...
)
{
  ...
  auto first = file_region.start_loc();
  auto last = file_region.end_loc();
