...
// @brief 宣言要素のリストの内容を出力する関数
void
VlDumperImpl::put_declarray_list(
  const char* label,
  const VlMgr& mgr,
  const std::vector<const VlDeclArray*>& declarray_list
)
{
...
void
VlDumperImpl::put_defparam_list(
  const char* label,
  const VlMgr& mgr,
...
void
VlDumperImpl::put_paramassign_list(
  const char* label,
  const VlMgr& mgr,
  const std::vector<const VlParamAssign*>& pa_list
)
{
...
// IEEE 1364-2001 p.644
void
VlDumperImpl::put_named_event_array(
  const char* label,
  const VpiHandle& handle
)
{
