...
)
{
  ...
  auto process_list = mgr.find_process_list(module);
...
void
VlDumperImpl::put_modulearray_list(
  const char* label,
  const VlMgr& mgr,
  const std::vector<const VlModuleArray*>& ma_list
)
{
