...
public:
  ...
  paramassign_list() const
  {
    ...
    std::vector<const PtConnection*> vec(n);
  ...
  std::vector<const PtDefParam*>
  defparam_list() const
  {
    auto n = defparam_num();
    std::vector<const PtDefParam*> vec(n);
    for ( SizeType i = 0; i < n; ++ i ) {
      vec[i] = defparam(i);
    }
    return vec;
  ...
  std::vector<const PtContAssign*>
  contassign_list() const
  {
    auto n = contassign_num();
    std::vector<const PtContAssign*> vec(n);
    for ( SizeType i = 0; i < n; ++ i ) {
      vec[i] = contassign(i);
    }
    return vec;
  ...
  std::vector<const PtInst*>
  inst_list() const
  {
    auto n = inst_num();
    std::vector<const PtInst*> vec(n);
    for ( SizeType i = 0; i < n; ++ i ) {
      vec[i] = inst(i);
    }
    return vec;
  ...
  {
    auto n = iohead_num();
    std::vector<const PtIOHead*> vec(n);
    for ( SizeType i = 0; i < n; ++ i ) {
      vec[i] = iohead(i);
    }
    return vec;
  ...
  {
    auto n = declhead_num();
    std::vector<const PtDeclHead*> vec(n);
    for ( SizeType i = 0; i < n; ++ i ) {
      vec[i] = declhead(i);
    }
    return vec;
  ...
  /// @brief ターミナルリストの取得
  std::vector<const PtExpr*>
  terminal_list() const
  {
    auto n = terminal_num();
    std::vector<const PtExpr*> vec(n);
    for ( SizeType i = 0; i < n; ++ i ) {
      vec[i] = terminal(i);
    }
    return vec;
  ...
  std::vector<const PtGenCaseItem*>
  caseitem_list() const
  {
    auto n = caseitem_num();
    std::vector<const PtGenCaseItem*> vec(n);
    for ( SizeType i = 0; i < n; ++ i ) {
      vec[i] = caseitem(i);
    }
    return vec;
