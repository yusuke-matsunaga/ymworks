...
public:
  ...
  ///
  /// kVpiPartSelect/kVpiBitSelect の時，意味を持つ．
  /// それ以外では常に false を返す．
  virtual
  bool
  ...
  ///
  /// それ以外では nullptr を返す．
  /// スコープへの参照が式で用いられるのはシステム関数/タスクの引数だけ
  virtual
  const VlScope*
  scope_obj() const = 0;
  ...
  ///
  /// primitiveへの参照が式で用いられるのはシステム関数/タスクの引数だけ
  virtual
  const VlPrimitive*
  ...
  ///
  /// multiple concatenation の時は operand(0) と等しい．
  virtual
  SizeType
  rep_num() const = 0;
