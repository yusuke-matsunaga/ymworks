...
public:
  ...
  /// @brief 要素のリストを返す．
  std::vector<const PtIOItem*>
  item_list() const
  {
    auto n = item_num();
    std::vector<const PtIOItem*> vec(n);
...
public:
  ...
  /// @brief 要素のリストを返す．
  std::vector<const PtDeclItem*>
  item_list() const
  {
    auto n = item_num();
    std::vector<const PtDeclItem*> vec(n);
    for ( SizeType i = 0; i < n; ++ i ) {
      vec[i] = item(i);
    }
    return vec;
...
public:
  ...
  /// @brief 要素のリストを返す．
  std::vector<const PtRange*>
  range_list() const
  {
    auto n = range_num();
    std::vector<const PtRange*> vec(n);
    for ( SizeType i = 0; i < n; ++ i ) {
      vec[i] = range(i);
    }
    return vec;
