...
)
{
  ...
  ElbConstantEnv env;
  return instantiate_expr(parent, env, ast_expr);
}

// @brief AstExpr からイベント式を生成する．
ElbExpr*
...
)
{
  ...
  case AstExpr::Primary:
    {
      auto expr = instantiate_primary(parent, env, ast_expr);
...
)
{
  if ( ast_expr_array.size() > 3 ) {
    throw std::logic_error{"ast_expr_array.size() > 3"};
  }
  ...
  try {
    ...
    expr_list.reserve(ast_expr_array.size());
    for ( auto ast_expr: ast_expr_array ) {
