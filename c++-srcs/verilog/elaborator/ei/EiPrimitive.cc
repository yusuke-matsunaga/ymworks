...
{
  return new EiPrimArray(head, ast_inst, ast_range, range);
}
...
{
  return new EiPrimArray(head, cell, ast_inst, ast_range, range);
}
...
// @brief ポート端子のリストを得る．
std::vector<const VlPrimTerm*>
EiPrimitive::prim_term_list() const
{
  std::vector<const VlPrimTerm*> term_list;
...
{
  mPortArray = std::vector<EiPrimTerm>(port_num);
