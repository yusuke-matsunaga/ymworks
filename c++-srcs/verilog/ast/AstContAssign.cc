
/// @file AstContAssign.cc
/// @brief AstContAssign の実装ファイル
/// @author Yusuke Matsunaga (松永 裕介)
///
/// Copyright (C) 2026 Yusuke Matsunaga
/// All rights reserved.

#include "ym/vl/AstContAssign.h"
#include "ym/vl/AstExpr.h"
#include "parser/PtContAssign.h"


BEGIN_NAMESPACE_YM_VERILOG

//////////////////////////////////////////////////////////////////////
// クラス AstContAssign
//////////////////////////////////////////////////////////////////////

// @brief 左辺式の取得
AstExpr
AstContAssign::lhs() const
{
  _check_ptr();
  return AstExpr(mPtr->lhs());
}

// @brief 右辺式の取得
AstExpr
AstContAssign::rhs() const
{
  _check_ptr();
  return AstExpr(mPtr->rhs());
}

// @brief 適切な値を持っている時 true を返す．
bool
AstContAssign::is_valid() const
{
  return mPtr != nullptr;
}

// @brief ファイル位置の取得
FileRegion
AstContAssign::file_region() const
{
  _check_ptr();
  return mPtr->file_region();
}

// @brief 比較用のユニークなキーを返す．
PtrIntType
AstContAssign::key() const
{
  return _key();
}

// @brief 次の要素を返す．
AstContAssign
AstContAssign::next() const
{
  _check_ptr();
  return AstContAssign(mPtr->link());
}

// @brief json_obj() の下請け関数
void
AstContAssign::json_sub(
  JsonValue& jobj
) const
{
  AstBase::json_sub(jobj);
  jobj.add("lhs", AstExpr(mPtr->lhs()).json_obj());
  jobj.add("rhs", AstExpr(mPtr->rhs()).json_obj());
}

END_NAMESPACE_YM_VERILOG
