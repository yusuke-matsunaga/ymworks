...
public:
  ...
  void
  set_searchpath(
    const PathList& searchpath = PathList() ///< [in] セットするサーチパス
  );
...
public:
  ...
  cur_rnumber() const
  {
    return strtod(mCurString.c_str(), static_cast<char**>(nullptr));
  }
...
public:
  ...
  void
  add_plugin(
    const std::string& name, ///< [in] プラグインを起動するコンパイラディレクティブ名
  ...
  bool
  erase_plugin(
    const std::string& name ///< [in] 削除するプラグイン名
  );
...
private:
  ...
  {
    mStringBuff += static_cast<char>(c);
  }
...
private:
  ...
  // 入力ファイルを管理するオブジェクト
  std::unique_ptr<InputMgr> mInputMgr;

  // 予約語辞書の参照
  const RsrvWordDic& mDic;

  // 内部状態用のリスト
  std::vector<std::unique_ptr<LexState>> mStates;
  ...
  // プラグイン用の辞書
  std::unique_ptr<LexPluginDict> mPluginDict;

  // コンテキスト
  Context mContext;

  // read_token の結果の文字列を格納するバッファ
  std::string mStringBuff;
