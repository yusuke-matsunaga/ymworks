...
public:
  ...
  PtiFactory*
  make_obj(
    const std::string& type, ///< [in] 実装クラスを指定する文字列
...
public:
  ...
  new_CmbUdp(
    ...
    const std::vector<const PtPort*>& port_array,     ///< [in] ポート名の配列
    const std::vector<const PtIOHead*>& io_array,     ///< [in] IOの配列
    const std::vector<const PtUdpEntry*>& entry_array ///< [in] テーブルエントリの配列
  ) = 0;
  ...
  new_UdpEntry(
    ...
    const std::vector<const PtUdpValue*>& input_array, ///< [in] 入力値の配列
  ...
  new_Module(
    ...
    const std::vector<const PtDeclHead*>& paramport_array, ///< [in] パラメータポートのリスト
    const std::vector<const PtPort*>& port_array,     ///< [in] ポートのリスト
    const std::vector<const PtIOHead*>& iodecl_array, ///< [in] 入出力のリスト
    const std::vector<const PtDeclHead*>& decl_array, ///< [in] 宣言のリスト
    const std::vector<const PtItem*>& item_array      ///< [in] 要素のリスト
  ) = 0;
  ...
  new_Port(
    const FileRegion& fr,   ///< [in] ファイル位置の情報
    const PtExpr* portref,  ///< [in] ポートに接続している式 (ポート参照式)
    const std::vector<const PtExpr*>& portref_array, ///< [in] ポートに接続している式 (ポート参照式)の配列
    const char* ext_name    ///< [in] ポート名 (空文字列の場合もある)
  ) = 0;
  ...
  new_DeclItem(
    const FileRegion& fr,                     ///< [in] ファイル位置の情報
    const char* name,                         ///< [in] 名前
    const std::vector<const PtRange*>& range_array ///< [in] 配列の各次元の範囲のリスト
  ) = 0;
  ...
  new_DefParamH(
    const FileRegion& fr,                       ///< [in] ファイル位置の情報
    const std::vector<const PtDefParam*>& elem_array ///< [in] 要素のリスト
  ) = 0;
  ...
  new_ContAssignH(
    ...
    const std::vector<const PtContAssign*>& elem_array ///< [in] 要素のリスト
  ) = 0;
  ...
  new_Task(
    ...
    const std::vector<const PtIOHead*>& iodecl_array, ///< [in] IO宣言のリスト
    const std::vector<const PtDeclHead*>& decl_array, ///< [in] 宣言のリスト
  ...
  new_GateH(
    ...
    const PtStrength* strength,		    ///< [in] 信号強度
    const PtDelay* delay,		    ///< [in] 遅延値
    const std::vector<const PtInst*>& elem_array ///< [in] 要素の配列
  ) = 0;
  ...
  new_MuH(
    const FileRegion& fr,                         ///< [in]ファイル位置の情報
    const char* def_name,                         ///< [in] 定義名
    const std::vector<const PtConnection*>& con_array, ///< [in] ポート割り当てリスト
    const std::vector<const PtInst*>& elem_array       ///< [in] 要素の配列
  ) = 0;
  ...
  new_Inst(
    ...
    const char* name,		 ///< [in] 名前
    const PtRange* range,        ///< [in] パース木の範囲定義
    const std::vector<const PtConnection*>& con_array ///< [in] ポート割り当ての配列
  ) = 0;
  ...
  new_GenIf(
    ...
    const std::vector<const PtDeclHead*>& then_decl_array, ///< [in] 条件が成り立った時に生成する宣言のリスト
    const std::vector<const PtItem*>& then_item_array,     ///< [in] 条件が成り立った時に生成する要素のリスト
    const std::vector<const PtDeclHead*>& else_decl_array, ///< [in] 条件が成り立たなかった時に生成する宣言のリスト
    const std::vector<const PtItem*>& else_item_array      ///< [in] 条件が成り立たなかった時に生成する要素のリスト
  ) = 0;
  ...
  new_GenCase(
    ...
    const std::vector<const PtGenCaseItem*>& item_array ///< [in] generate case item のリスト
  ) = 0;
  ...
  new_GenCaseItem(
    const FileRegion& fr,                        ///< [in] ファイル位置の情報
    const std::vector<const PtExpr*>& label_array,    ///< [in] 比較式のリスト
    const std::vector<const PtDeclHead*>& decl_array, ///< [in] 宣言のリスト
    const std::vector<const PtItem*>& item_array      ///< [in] 要素のリスト
  ) = 0;
  ...
  new_SpecItem(
    ...
    const std::vector<const PtExpr*>& terminal_array ///< [in] 端子のリスト
  ) = 0;
  ...
  new_PathDecl(
    const FileRegion& fr,                      ///< [in] ファイル位置の情報
    int edge,
    const std::vector<const PtExpr*>& input_array,
    int input_pol,
    VpiPathType op,
    const std::vector<const PtExpr*>& output_array,
    int output_pol,
    const PtExpr* expr,
  ...
  new_Enable(
    ...
    const std::vector<const PtExpr*>& arg_array ///< [in] 引数のリスト
  ) = 0;
  ...
  new_Case(
    const FileRegion& fr,                           ///< [in] ファイル位置の情報
    const PtExpr* expr,                             ///< [in] 条件式
    const std::vector<const PtCaseItem*>& caseitem_array ///< [in] case item のリスト
  ) = 0;
  ...
  new_CaseItem(
    const FileRegion& fr,                     ///< [in] ファイル位置の情報
    const std::vector<const PtExpr*>& label_array, ///< [in] ラベルのリスト
    const PtStmt* body                        ///< [in] 本体のステートメント
  ) = 0;
  ...
  new_ParBlock(
    const FileRegion& fr,                 ///< [in] ファイル位置の情報
    const std::vector<const PtStmt*>& stmt_lit ///< [in] 本体のステートメントのリスト
  ) = 0;
  ...
  new_Concat(
    const FileRegion& fr,                   ///< [in] ファイル位置の情報
    const std::vector<const PtExpr*>& expr_array ///< [in] オペランドのリスト
  ) = 0;
  ...
  new_MultiConcat(
    const FileRegion& fr,                   ///< [in] ファイル位置の情報
    const std::vector<const PtExpr*>& expr_array ///< [in] 結合するオペランドのリスト
  ) = 0;
  ...
  new_Primary(
    const FileRegion& fr,                    ///< [in] ファイル位置の情報
    const char* name,                        ///< [in] 識別子名
    const std::vector<const PtExpr*>& index_array ///< [in] インデックスのリスト
  ) = 0;
  ...
  new_EventControl(
    const FileRegion& fr,                    ///< [in] ファイル位置の情報
    const std::vector<const PtExpr*>& event_array ///< [in] イベントのリスト
  ) = 0;
  ...
  new_RepeatControl(
    const FileRegion& fr,                    ///< [in] ファイル位置の情報
    const PtExpr* expr,                      ///< [in] 繰り返し数を表す式
    const std::vector<const PtExpr*>& event_array ///< [in] 繰り返しの単位となるイベントのリスト
  ) = 0;
  ...
  new_AttrInst(
    const FileRegion& fr,                     ///< [in] ファイル位置の情報
    const std::vector<const PtAttrSpec*>& as_array ///< [in] attribute spec のリスト
  ) = 0;
