...
TEST_F(ParserTest, Generate)
{
  ...
  EXPECT_EQ( PtItemType::Generate, item->type() );
...
TEST_F(ParserTest, GenBlock1)
{
  ...
  EXPECT_EQ( PtItemType::GenBlock, item0->type() );
...
TEST_F(ParserTest, GenIf)
{
  ...
  auto item0 = item->item(0);
  EXPECT_EQ( fr0, item0->file_region() );
  EXPECT_EQ( PtItemType::GenIf, item0->type() );
  EXPECT_EQ( nullptr, item0->name() );
  EXPECT_THROW( item->item(1),
		std::out_of_range );
  EXPECT_THROW( item0->prim_type(),
...
TEST_F(ParserTest, GenIfElse)
{
  ...
  EXPECT_EQ( std::vector<const PtDeclHead*>{net_decl2},
	     item0->else_declhead_list() );
  EXPECT_EQ( 0, item0->else_item_num() );
  EXPECT_THROW( item0->else_item(0),
		std::out_of_range );
...
TEST_F(ParserTest, GenCase)
{
  ...
  auto caseitem_list = parser.new_list<const PtGenCaseItem>();
  ...
  // default の場合
  auto null_list = parser.new_list<const PtExpr>();
  ...
  EXPECT_EQ( PtItemType::GenCase, item0->type() );
  EXPECT_EQ( nullptr, item0->name() );
  EXPECT_THROW( item->item(1),
		std::out_of_range );
  EXPECT_THROW( item0->prim_type(),
  ...
  auto exp_caseitem_list = std::vector<const PtGenCaseItem*>{caseitem0, caseitem1};
...
TEST_F(ParserTest, GenFor)
{
  ...
  auto item0 = item->item(0);
  EXPECT_EQ( fr0, item0->file_region() );
  EXPECT_EQ( PtItemType::GenFor, item0->type() );
