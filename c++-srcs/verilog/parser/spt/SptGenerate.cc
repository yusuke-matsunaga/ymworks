...
) : mDeclArray{std::move(declhead_array)},
...
SptGenIf::SptGenIf(
  ...
  PtiItemArray&& else_item_array
    ...
    mCond{cond},
    mThenBody{std::move(then_declhead_array),
	      std::move(then_item_array)},
    mElseBody{std::move(else_declhead_array),
