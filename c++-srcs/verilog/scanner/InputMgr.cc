...
)
{
  ...
  // 本当のパス名
  auto realname = std::filesystem::canonical(pathname);

  std::ifstream tmp_in(realname);
  if ( !tmp_in ) {
    return false;
  }
  mFsStack.push_back(std::ifstream()); // ダミー
  std::ifstream& in = mFsStack.back();
  in.swap(tmp_in);

  auto scanner = new Scanner{in, {realname, parent_file}};
  mFileStack.push_back(std::unique_ptr<Scanner>{scanner});
