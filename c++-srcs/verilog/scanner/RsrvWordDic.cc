...
) const
{
  ...
  for ( auto cell = mTable1[pos]; cell; cell = cell->mLink1 ) {
    if ( str == cell->mStr ) {
      return cell->mTok;
    }
  }
...
) const
{
  ...
  for ( auto cell = mTable2[pos]; cell; cell = cell->mLink2 ) {
    if ( token == cell->mTok ) {
      return cell->mStr;
    }
  }
  std::ostringstream buf;
  buf << "Token#" << token << " is not a valid token";
...
)
{
  ...
  case COMMENT2:       s << "COMMENT2: " << std::endl << str; break;
