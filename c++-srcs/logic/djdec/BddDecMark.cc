...
int
BddDecMark::get_boundary(
  BddDecEdge edge,
  std::vector<const BddDecNode*>& or_list,
  std::vector<const BddDecNode*>& xor_list
)
{
  auto node = edge.node();
  if ( bmark(node) == 0 ) {
...
)
{
  ...
  if ( val != 3 ) {
    if ( val == 1 ) {
      inputs.push_back(BddDecEdge{node});
    }
    else {
