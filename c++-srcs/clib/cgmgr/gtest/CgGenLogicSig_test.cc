...
)
{
  ...
  // mFunc の真理値表を取り出す．
  std::vector<std::vector<bool>> func_table_list(no2);
  std::vector<std::vector<bool>> tristate_table_list(no2);
	...
	for ( PermGen ipg(ni, ni); !ipg.is_end(); ++ ipg ) {
	  ...
	  for ( PermGen opg(no, no); !opg.is_end(); ++ opg ) {
		...
		bpin_map[i] = ClibPinMap(pos, bpol_list[pos]);
	      ...
	      std::vector<std::vector<bool>> xfunc_table_list(no2);
	      std::vector<std::vector<bool>> xtristate_table_list(no2);
	      for ( SizeType i = 0; i < no; ++ i ) {
		SizeType pos = opg(i);
		...
		for ( SizeType i = 0; i < no2; ++ i ) {
		  std::cout << "#" << i
			    << ": " << to_hex(xfunc_table_list[i])
			    << ": " << to_hex(xtristate_table_list[i])
			    << std::endl;
		}
	      }
	      // 代表シグネチャを求める．
