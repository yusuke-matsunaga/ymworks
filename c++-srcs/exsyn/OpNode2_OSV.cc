...
{
  mSel0 = solver.new_ordered_set(0, cand_num - 1);
  mSel1 = solver.new_ordered_set(0, cand_num - 1);
...
void
OpNode2_OSV::add_no_reapplication_cond(
  SatSolver& solver,
  const OpNode* op2
) const
{
...
void
OpNode2_OSV::add_ng_operand_cond(
  SatSolver& solver,
  SizeType opr0,
  SizeType opr1
) const
{
  auto lit1 = mSel0.var(opr0);
  auto lit2 = mSel1.var(opr1);
  solver.add_clause(~lit1, ~lit2);
}

// @brief 自分より前の演算ノードに対する制約を追加する．
void
OpNode2_OSV::add_op_order_cond(
  SatSolver& solver,
  const OpNode* op
) const
{
  auto op_osv = reinterpret_cast<const OpNode2_OSV*>(op);
...
void
OpNode2_OSV::add_use_lit(
  SizeType op_id,
  std::vector<SatLiteral>& lit_list
) const
{
  if ( op_id <= mSel0.max() ) {
    auto lit0 = mSel0.var(op_id);
    lit_list.push_back(lit0);
  }
  if ( op_id <= mSel1.max() ) {
    auto lit1 = mSel1.var(op_id);
