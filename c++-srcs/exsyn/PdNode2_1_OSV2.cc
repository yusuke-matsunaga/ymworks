...
/// @file PdNode2_1_OSV2.cc
/// @brief PdNode2_1_OSV2 の実装ファイル
...
void
PdNode2_1_OSV2::add_no_reapplication_cond(
  SatSolver& solver,
  const OpNode* op2
) const
{
...
void
PdNode2_1_OSV2::add_ng_operand_cond(
  SatSolver& solver,
  SizeType opr0,
  SizeType opr1
) const
...
void
PdNode2_1_OSV2::add_op_order_cond(
  SatSolver& solver,
  const OpNode* op
) const
{
  auto op_osv = reinterpret_cast<const PdNode2_1_OSV2*>(op);
  solver.add_le(mSel0, op_osv->mSel0);
}

// @brief SATモデルから入力の選択結果を得る．
std::vector<SizeType>
PdNode2_1_OSV2::get_opr(
