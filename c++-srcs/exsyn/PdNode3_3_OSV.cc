...
/// @file PdNode3_3_OSV.cc
/// @brief PdNode3_3_OSV の実装ファイル
...
) const
{
  ...
  for ( SizeType sel0 = 0; sel0 < input_num() - 2; ++ sel0 ) {
	...
	// 演算ノードの入力と出力に関する条件
	add_op_cond(solver,
		    ival_list[sel0], ival_list[sel1], ival_list[sel2],
		    ovar);
      }
    }
  }
...
void
PdNode3_3_OSV::add_op_order_cond(
  SatSolver& solver,
  const OpNode* op
) const
{
  auto op_osv = reinterpret_cast<const PdNode3_3_OSV*>(op);
