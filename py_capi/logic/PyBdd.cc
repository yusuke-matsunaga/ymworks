...
)
{
  ...
  try {
    ...
    if ( PyBddCube::Check(arg_obj) ) {
      if ( inv ) {
        PyErr_SetString(PyExc_TypeError, "inv should not be specified with BddCube");
        return nullptr;
      }
...
)
{
  ...
  try {
    return PyBdd::ToPyObject(ite(val1, val2, val3));
  }
  catch ( std::exception err ) {
    std::ostringstream buf;
    buf << "exception" << ": " << err.what();
...
)
{
  ...
  try {
    return PyBdd::ToPyObject(simplify(on, dc));
  }
  catch ( std::exception err ) {
    std::ostringstream buf;
    buf << "exception" << ": " << err.what();
...
)
{
  auto& val = PyBdd::_get_ref(self);
  try {
    return PyBddCube::ToPyObject(val.get_zeropath());
  }
  catch ( std::exception err ) {
    std::ostringstream buf;
    buf << "exception" << ": " << err.what();
...
)
{
  ...
  if ( bdd_list_obj != nullptr ) {
    if ( !PyList<Bdd, PyBdd>::FromPyObject(bdd_list_obj, bdd_list) ) {
      PyErr_SetString(PyExc_ValueError, "could not convert to std::vector<Bdd>");
      return nullptr;
    }
  }
  try {
    return PyLong::ToPyObject(bdd_size(bdd_list));
  }
  catch ( std::exception err ) {
    std::ostringstream buf;
    buf << "exception" << ": " << err.what();
...
PyMethodDef methods[] = {
  ...
  {"bdd_size",
   reinterpret_cast<PyCFunction>(bdd_size),
   METH_VARARGS | METH_KEYWORDS | METH_STATIC,
   PyDoc_STR("return node size of the BDDs")},
  {"is_identical",
...
// getter/setter定義
PyGetSetDef getsets[] = {
  {"size", get_size, nullptr, PyDoc_STR("size"), nullptr},
  {"mgr", get_mgr, nullptr, PyDoc_STR("BddMgr"), nullptr},
  {"root_inv", get_root_inv, nullptr, PyDoc_STR("True if root edge is inverted"), nullptr},
  // end-marker
