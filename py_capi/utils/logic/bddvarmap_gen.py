...
""" PyBddVarMap を生成するスクリプト

:file: bddvarmap_gen.py
...
class BddVarMapGen(PyObjGen):

    def __init__(self):
        super().__init__(classname='Bdd::VarMap',
                         pyclassname='PyBddVarMap',
                         pyname='BddVarMap',
                         namespace='YM',
                         header_include_files=['ym/Bdd.h',
                                               'ym/BddVar.h'],
                         source_include_files=['pym/PyBddVarMap.h',
                                               'pym/PyBddVar.h',
                                               'pym/PyBddLit.h'])
        ...
        def new_body(writer):
            ...
            writer.write_line('new (&my_obj->mVal) Bdd::VarMap();')
            with writer.gen_if_block('obj != nullptr'):
                writer.gen_auto_assign('emsg',
                                       '"1st argument should be a sequence of (BddVar, BddLit)"')
                ...
                with writer.gen_foreach_sequence_block('obj1', 'obj'):
                    ...
                    with writer.gen_if_block('!PyBddVar::Check(var_obj)'):
                        writer.write_line('Py_DecRef(obj1);')
                        writer.gen_type_error('emsg')
                    with writer.gen_if_block('!PyBddLit::Check(lit_obj)'):
                        writer.write_line('Py_DecRef(obj1);')
                        writer.gen_type_error('emsg')
                    writer.gen_autoref_assign('var', 'PyBddVar::_get_ref(var_obj)')
                    writer.gen_autoref_assign('lit', 'PyBddLit::_get_ref(lit_obj)')
        ...
        def mp_subscript(writer):
            ...
            writer.gen_return_pyobject('PyBddLit', 'val.at(var)')
        def mp_ass_subscript(writer):
            with writer.gen_if_block('!PyBddVar::Check(key)'):
                writer.gen_type_error('"1st argument should be BddVar"',
                                      error_val=-1)
            with writer.gen_if_block('!PyBddLit::Check(obj)'):
                writer.gen_type_error('"2nd argument should be BddLit"',
                                      error_val=-1)
            writer.gen_autoref_assign('var', 'PyBddVar::_get_ref(key)')
            writer.gen_autoref_assign('lit', 'PyBddLit::_get_ref(obj)')
