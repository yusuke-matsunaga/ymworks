...
def test_object0():
    ...
    js_obj3 = JsonValue.object()
    assert js_obj == js_obj3

def test_array0():
    js_obj = JsonValue.array()
...
def test_array1():
    ...
    assert js_obj[-1] == js_obj3
    assert js_obj[-2] == js_obj2
    assert js_obj[-3] == js_obj1

    with pytest.raises(Exception) as e:
        js_obj[3]
    assert e.type == ValueError
    assert str(e.value) == "out of range: pos is out of range"

    for i, elem in enumerate(js_obj.get_array()):
        assert elem == js_obj[i]
...
def test_object1():
    ...
    with pytest.raises(Exception) as e:
        js_obj["abc"]
    assert e.type == ValueError
    assert str(e.value) == "out of range: abc: invalid key"

    key_list = js_obj.key_list()
    assert len(key_list) == 3
    assert "key1" in key_list
    assert "key2" in key_list
    assert "key3" in key_list

    item_list = js_obj.item_list()
    assert len(item_list) == 3
    assert ("key1", js_obj1) in item_list
    assert ("key2", js_obj2) in item_list
    assert ("key3", js_obj3) in item_list
...
def test_object2():
    ...
    js_obj = JsonValue.object()
    js_obj.add_with_key("key1", js_obj1)
    js_obj.add_with_key("key2", js_obj2)
    js_obj.add_with_key("key3",  js_obj3)
