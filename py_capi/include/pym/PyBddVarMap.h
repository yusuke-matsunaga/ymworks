...
/// @brief Bdd::VarMap を Python から使用するための拡張
///
...
public:
  ...
  using ElemType = Bdd::VarMap;

public:

  // このクラスは Conv を持たない．

  /// @brief PyObject* から Bdd::VarMap を取り出すファンクタクラス
  struct Deconv {
    bool
    operator()(
...
public:
  ...
  {
    ElemType val;
    if ( PyBddVarMap::FromPyObject(obj, val) ) {
      return val;
    }
  ...
  /// @brief Bdd::VarMap を表す PyObject から Bdd::VarMap を取り出す．
  /// @return Bdd::VarMap を返す．
  ///
