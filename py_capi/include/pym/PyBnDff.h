...
/// @brief BnDff を Python から使用するための拡張
///
...
public:
  ...
  using ElemType = BnDff;

public:

  /// @brief BnDff を PyObject* に変換するファンクタクラス
  struct Conv {
    PyObject*
    operator()(
  ...
  /// @brief PyObject* から BnDff を取り出すファンクタクラス
  struct Deconv {
    bool
    operator()(
...
public:
  ...
  {
    ElemType val;
    if ( PyBnDff::FromPyObject(obj, val) ) {
      return val;
    }
