...
/// @brief BnFunc を Python から使用するための拡張
///
...
public:
  ...
  using ElemType = BnFunc;

public:

  /// @brief BnFunc を PyObject* に変換するファンクタクラス
  struct Conv {
    PyObject*
    operator()(
  ...
  /// @brief PyObject* から BnFunc を取り出すファンクタクラス
  struct Deconv {
    bool
    operator()(
...
public:
  ...
  {
    ElemType val;
    if ( PyBnFunc::FromPyObject(obj, val) ) {
      return val;
    }
    PyErr_SetString(PyExc_TypeError, "Could not convert to BnFunc");
    return val;
  }
