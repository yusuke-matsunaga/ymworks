...
/// @brief ClibResistance を Python から使用するための拡張
///
...
public:
  ...
  using ElemType = ClibResistance;

public:

  /// @brief ClibResistance を PyObject* に変換するファンクタクラス
  struct Conv {
    PyObject*
    operator()(
  ...
  /// @brief PyObject* から ClibResistance を取り出すファンクタクラス
  struct Deconv {
    bool
    operator()(
...
public:
  ...
  {
    ElemType val;
    if ( PyClibResistance::FromPyObject(obj, val) ) {
      return val;
    }
    PyErr_SetString(PyExc_TypeError, "Could not convert to ClibResistance");
    return val;
  }
