...
/// @brief Bdd::ComposeMap を Python から使用するための拡張
///
...
public:
  ...
  using ElemType = Bdd::ComposeMap;

public:

  // このクラスは Conv を持たない．

  /// @brief PyObject* から Bdd::ComposeMap を取り出すファンクタクラス
  struct Deconv {
    bool
    operator()(
...
public:
  ...
  {
    ElemType val;
    if ( PyBddCompMap::FromPyObject(obj, val) ) {
      return val;
    }
  ...
  /// @brief Bdd::ComposeMap を表す PyObject から Bdd::ComposeMap を取り出す．
  /// @return Bdd::ComposeMap を返す．
  ///
