...
/// @brief ClibLut を Python から使用するための拡張
///
...
public:
  ...
  using ElemType = ClibLut;

public:

  /// @brief ClibLut を PyObject* に変換するファンクタクラス
  struct Conv {
    PyObject*
    operator()(
  ...
  /// @brief PyObject* から ClibLut を取り出すファンクタクラス
  struct Deconv {
    bool
    operator()(
...
public:
  ...
  {
    ElemType val;
    if ( PyClibLut::FromPyObject(obj, val) ) {
      return val;
    }
    PyErr_SetString(PyExc_TypeError, "Could not convert to ClibLut");
    return val;
  }
