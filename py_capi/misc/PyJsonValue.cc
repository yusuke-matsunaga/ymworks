...
static const char* EMSG_KEY_NOT_FOUND = "key is not found";
...
{
  try {
    return PyJsonValue::ToPyObject(JsonValue::object());
  }
...
{
  try {
    return PyJsonValue::ToPyObject(JsonValue::array());
  }
  catch ( std::exception err ) {
    std::ostringstream buf;
    buf << "exception" << ": " << err.what();
...
)
{
  ...
  try {
    ...
    return PyList<JsonValue, PyJsonValue>::ToPyObject(val.get_array());
  }
  catch ( std::exception err ) {
    std::ostringstream buf;
    buf << "exception" << ": " << err.what();
...
)
{
  ...
  if ( value_obj != nullptr ) {
    if ( !PyJsonValue::FromPyObject(value_obj, value) ) {
...
PyMethodDef methods[] = {
  ...
  {"object",
   object,
   METH_NOARGS | METH_STATIC,
   PyDoc_STR("make a dictionary type object")},
  {"array",
   array,
   METH_NOARGS | METH_STATIC,
   PyDoc_STR("make an array type object")},
  {"is_null",
   is_null,
   METH_NOARGS,
  ...
  {"key_list",
   key_list,
   METH_NOARGS,
   PyDoc_STR("return list of keys")},
  {"item_list",
   item_list,
   METH_NOARGS,
   PyDoc_STR("return list of items")},
  {"add",
   reinterpret_cast<PyCFunction>(add),
   METH_VARARGS | METH_KEYWORDS,
   PyDoc_STR("add an array element")},
  {"add_with_key",
   reinterpret_cast<PyCFunction>(add_with_key),
   METH_VARARGS | METH_KEYWORDS,
   PyDoc_STR("add a dictionary element")},
  // end-marker
